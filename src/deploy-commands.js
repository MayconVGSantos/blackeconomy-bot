...
async function registerCommands() {
  try {
    ...
    if (!process.env.DISCORD_TOKEN) {
      console.error('Token do Discord não encontrado no arquivo .env! Verifique se o arquivo .env existe e contém DISCORD_TOKEN=seu_token');
      return;
    }

    // Verificar se o clientId está disponível
    if (!process.env.CLIENT_ID) {
      console.error('Client ID não encontrado no arquivo .env! Verifique se o arquivo .env contém CLIENT_ID=seu_client_id');
      return;
    }
    ...
    console.log(`Token disponível: ${process.env.DISCORD_TOKEN ? 'Sim (primeiros 10 caracteres: ' + process.env.DISCORD_TOKEN.substring(0, 10) + '...)' : 'Não'}`);

    // Configurar REST API diretamente com o token do .env
    const rest = new REST().setToken(process.env.DISCORD_TOKEN);

    // Registrar os comandos para a aplicação
    console.log(`Tentando registrar comandos com Client ID: ${process.env.CLIENT_ID}`);
