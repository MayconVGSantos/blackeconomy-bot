...
export async function execute(interaction) {
  try {
    ...
    const resultado = await casinoService.exchangeChipsForMoney(userId, quantidade);
    
    if (!resultado.success) {
      const embedErro = embedUtils.criarEmbedErro({
        usuario: interaction.user.username,
        titulo: 'Erro na Troca',
        mensagem: resultado.error || 'Ocorreu um erro ao trocar suas fichas por dinheiro.'
      });
    ...
    const embed = new EmbedBuilder()
      ...
      .setDescription(`Você trocou **${quantidade} fichas** por **R$${resultado.amount.toFixed(2)}**.`)
      .addFields(
        { name: '📊 Taxa de Conversão', value: `1 ficha = R$10.00`, inline: true },
        { name: '💸 Taxa Cobrada', value: `R$${resultado.taxa.toFixed(2)} (10%)`, inline: true },
        { name: '🪙 Fichas Restantes', value: `${resultado.newChips}`, inline: true },
        { name: '💰 Novo Saldo', value: `R$${resultado.newBalance.toFixed(2)}`, inline: true }
      )
