...
import { 
  ...
  export const data = new SlashCommandBuilder()
    ...
    .addIntegerOption(option => 
      option.setName('coluna')
        .setDescription('Coluna para apostar (1, 2 ou 3) se escolheu tipo "coluna"')
        .setRequired(false)
        .addChoices(
    ...
    try {
      ...
      switch (tipoAposta) {
        ...
        case 'cor':
          ...
          descricaoAposta = `Cor: ${valorAposta === 'red' ? '🔴 Vermelho' : '⚫ Preto'}`;
          break;
      ...
      // Registrar a aposta
      const apostaRegistrada = await casinoService.registerBet(userId, aposta, 'roleta');
      ...
      switch (tipoAposta) {
        ...
        case 'paridade':
          ...
          if (resultado.number === 0) {
            ...
            ganhou = (valorAposta === 'par' && ehPar) || (valorAposta === 'impar' && !ehPar);
          }
      ...
      const novasFichas = await casinoService.registerResult(userId, aposta, ganho, 'roleta');
      ...
      const detalhesResultado = [
        ...
        `Cor: ${resultado.color === 'red' ? '🔴 Vermelho' : (resultado.color === 'black' ? '⚫ Preto' : '🟢 Verde')}`,
      ...
      const embed = new EmbedBuilder()
        ...
        .addFields(
          ...
          { name: '💰 Aposta', value: `${aposta} fichas em ${descricaoAposta}`, inline: true },
      ...
      collector.on('collect', async i => {
        if (i.user.id !== userId) {
          await i.reply({ content: 'Você não pode usar estes controles.', ephemeral: true });
          return;
        }
        ...
        if (i.componentType === ComponentType.Button) {
          if (i.customId === 'roleta_again') {
            ...
            newCommand.options.getInteger = (name) => {
              ...
              if (name === 'numero' && tipoAposta === 'numero') return valorAposta;
              if (name === 'duzia' && tipoAposta === 'duzia') return valorAposta;
              if (name === 'coluna' && tipoAposta === 'coluna') return valorAposta;
              return null;
            };
            ...
            newCommand.options.getString = (name) => {
              ...
              if (name === 'paridade' && tipoAposta === 'paridade') return valorAposta;
              return null;
            };
          ...
          if (i.customId === 'roleta_nova_aposta') {
            ...
            newCommand.options.getString = (name) => {
              if (name === 'tipo') {
                ...
                if (opcaoSelecionada === 'par' || opcaoSelecionada === 'impar') return 'paridade';
              ...
              if (name === 'paridade' && (opcaoSelecionada === 'par' || opcaoSelecionada === 'impar')) {
