...
export async function execute(interaction) {
  try {
    ...
    // Registrar a aposta
    const apostaRegistrada = await casinoService.registerBet(userId, aposta, 'slots');
...
const resultado = casinoService.generateSlotsResult(userId);
    ...
    const novasFichas = await casinoService.registerResult(userId, aposta, ganho, 'slots');
...
${ganho > 0 ? `**${resultado.winType}** Você ganhou **${ganho} fichas**!` : 'Que pena! Você não ganhou nada.'}`)
      .addFields(
        { name: '💰 Aposta', value: `${aposta} fichas`, inline: true },
        { name: '🪙 Multiplicador', value: `${resultado.multiplier}x`, inline: true },
