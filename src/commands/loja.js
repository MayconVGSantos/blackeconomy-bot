...
import { 
   ...
   */
   async function showMainMenu(interaction, userId) {
    ...
    const embed = new EmbedBuilder()
      ...
      .setDescription(`Bem-vindo à loja, ${interaction.user.username}!\nEscolha uma categoria para ver os itens disponíveis.`)
      ...
      .addFields(
        ...
        { name: '📋 Categorias Disponíveis', value: categories.map(cat => 
          `${storeItemsService.getCategoryIcon(cat)} ${storeItemsService.getCategoryDisplayName(cat)}`
        ).join('\n'), inline: true }
      )
   ...
   * @returns {Promise<void>}
   */
   async function showCategoryItems(interaction, userId, category, existingReply = null) {
    // Obter saldo do usuário
    const userData = await firebaseService.getUserData(userId);
    const saldo = userData.saldo || 0;
    ...
    if (items.length === 0) {
      const embed = new EmbedBuilder()
        ...
        .setDescription(`Não há itens disponíveis na categoria ${storeItemsService.getCategoryDisplayName(category)}.`);
   ...
   ${item.description}`;
    ...
    const embed = new EmbedBuilder()
      .setColor(0xFFD700) // Dourado
      .setTitle(`${storeItemsService.getCategoryIcon(category)} ${storeItemsService.getCategoryDisplayName(category)}`)
      .setDescription(itemsDescription)
      .addFields(
        { name: '💰 Seu Saldo', value: `R$${saldo.toFixed(2)}`, inline: true }
      )
      .setFooter({ text: 'Use os botões abaixo para comprar itens ou voltar ao menu.' })
      .setTimestamp();
    ...
    collector.on('collect', async i => {
      ...
      if (customId === 'store_close') {
        const closedEmbed = new EmbedBuilder()
          ...
          .setDescription('Você fechou a loja. Use `/loja` novamente quando quiser comprar algo.');
   ...
   async function buyItem(interaction, userId, itemId, buttonInteraction) {
    try {
      ...
      if (!item) {
        ...
        return buttonInteraction.editReply({ embeds: [embedErro], components: [] });
      }
      ...
      if (saldo < item.price) {
        const embedErro = embedUtils.criarEmbedErro({
          ...
          mensagem: `Você tem apenas R$${saldo.toFixed(2)}, mas o item custa R$${item.price.toFixed(2)}.`
        });
      ...
      // Processar a compra
      // 1. Debitar o saldo
      const novoSaldo = await firebaseService.updateUserBalance(userId, -item.price);
      ...
      const embedSucesso = new EmbedBuilder()
        ...
        .setDescription(`Você comprou **${item.icon} ${item.name}** por **R$${item.price.toFixed(2)}**.`)
        .addFields(
          { name: '💰 Novo Saldo', value: `R$${novoSaldo.toFixed(2)}`, inline: true }
        )
        .setFooter({ text: `Comprado por ${interaction.user.username}` })
        .setTimestamp();
      
      await buttonInteraction.followUp({ embeds: [embedSucesso], ephemeral: true });
      ...
      const embedErro = embedUtils.criarEmbedErro({
        ...
        mensagem: 'Ocorreu um erro ao processar sua compra. Tente novamente mais tarde.'
      });
