...
import { 
  ...
  export const data = new SlashCommandBuilder()
    ...
    .addIntegerOption(option => 
      option.setName('numero')
        .setDescription('Número que você aposta que sairá na soma dos dados (2-12)')
        .setRequired(true)
        .setMinValue(2)
        .setMaxValue(12)
    );
  ...
  export async function execute(interaction) {
    try {
      ...
      // Registrar a aposta
      const apostaRegistrada = await casinoService.registerBet(userId, aposta, 'dados');
      ...
      const novasFichas = await casinoService.registerResult(userId, aposta, ganho, 'dados');
      ...
      const embed = new EmbedBuilder()
        ...
        .addFields(
          { name: '🎯 Número Apostado', value: `${numeroApostado}`, inline: true },
          { name: '🎲 Dados', value: `${resultado.dice1} + ${resultado.dice2} = ${somaDados}`, inline: true },
