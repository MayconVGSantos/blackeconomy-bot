...
export async function execute(interaction) {
  try {
    ...
    let probabilidadeSucesso = userData.saldo / (targetData.saldo + userData.saldo);
    ...
    if (sucesso) {
      ...
      novoSaldoVitima = await firebaseService.updateUserBalance(targetUser.id, -valor);
    ...
    let conteudo;
    try {
      conteudo = await geminiClient.gerarRespostaRoubo(valor, sucesso, targetUser.username);
    } catch (error) {
      console.error('Erro ao gerar resposta com Gemini:', error);
      conteudo = sucesso 
        ? `Você roubou R$${valor.toFixed(2)} de ${targetUser.username}.`
        : `Você foi pego tentando roubar ${targetUser.username} e pagou uma multa de R$${valor.toFixed(2)}.`;
    }
