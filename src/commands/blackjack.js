// blackjack.js - Com formatação brasileira
import {
  SlashCommandBuilder,
  EmbedBuilder,
  ActionRowBuilder,
...
export async function execute(interaction) {
  try {
    ...
    // Start game with the provided interaction and bet amount
    await startBlackjackGame(interaction, userId, aposta);
  } catch (error) {
...
/**
 * Inicia ou reinicia um jogo de blackjack
 * @param {Interaction} interaction - Interação do Discord
 * @param {string} userId - ID do usuário
 * @param {number} aposta - Valor da aposta
 * @param {boolean} isNewGame - Se é um novo jogo iniciado de um botão
 */
async function startBlackjackGame(
  ...
  try {
    ...
    if (playerScore === 21) {
      return await handleNaturalBlackjack(
        interaction,
        userId,
        aposta,
        playerHand,
    ...
    console.error("Erro ao iniciar jogo de blackjack:", error);
    ...
    const embedErro = embedUtils.criarEmbedErro({
      ...
      titulo: "Erro no Jogo",
      mensagem:
        "Ocorreu um erro ao iniciar o jogo. Tente novamente mais tarde.",
    });
    ...
    .addFields(
      { name: "🎰 Aposta", value: `${aposta} fichas`, inline: true },
      {
        ...
        value: `${dealerCards}\nPontuação: ${
          hideDealer
            ? dealerScore
            : casinoService.calculateBlackjackScore(dealerHand)
        }`,
        inline: false,
      },
      {
...
async function handleNaturalBlackjack(
  ...
  collector.on("collect", async (i) => {
    ...
    if (i.customId === "blackjack_again") {
      collector.stop();
      // Iniciar novo jogo com a mesma aposta
      await startBlackjackGame(i, userId, aposta, true);
    } else if (i.customId === "blackjack_double") {
      collector.stop();
      // Iniciar novo jogo com aposta dobrada
      await startBlackjackGame(i, userId, aposta * 2, true);
    }
  });
...
async function handlePlayerHit(
  ...
  gameState.playerScore = casinoService.calculateBlackjackScore(
    gameState.playerHand
  );
  ...
  if (gameState.playerScore > 21) {
    ...
    // Criar novo coletor para os botões
    const newCollector = i.message.createMessageComponentCollector({
      componentType: ComponentType.Button,
      time: 60000, // 1 minuto
    });

    newCollector.on("collect", async (btn) => {
      ...
      if (btn.customId === "blackjack_again") {
        newCollector.stop();
        // Iniciar novo jogo com a mesma aposta
        await startBlackjackGame(btn, userId, aposta, true);
      } else if (btn.customId === "blackjack_double") {
        newCollector.stop();
        // Iniciar novo jogo com aposta dobrada
        await startBlackjackGame(btn, userId, aposta * 2, true);
      }
    });
 ...
 * @returns {Promise<void>}
 */
async function handlePlayerStand(
  gameState,
  i,
  collector,
  username,
  ...
  while (gameState.dealerScore < 17) {
    ...
    gameState.dealerScore = casinoService.calculateBlackjackScore(
      gameState.dealerHand
    );
  }
