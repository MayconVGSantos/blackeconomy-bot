...
export async function execute(interaction) {
  try {
    ...
    if (category) {
      return await showCategoryItems(
        interaction,
        targetUser,
        category,
        inventory
...
async function showInventorySummary(interaction, targetUser, inventory) {
  ...
  // Campos para categorias de itens (com ícones corrigidos)
  if (itemCounts.casino > 0) {
    fields.push({
      name: "🎰 Cassino",
      value: `${itemCounts.casino} item(s)`,
      inline: true,
    });
  }

  if (itemCounts.consumiveis > 0) {
    fields.push({
      name: "🧪 Consumíveis",
      value: `${itemCounts.consumiveis} item(s)`,
      inline: true,
    });
  }
  ...
  if (fields.length === 0) {
    fields.push({
      ...
      value:
        "Você ainda não possui nenhum item. Use o comando `/loja` para comprar itens.",
      inline: false,
    });
  }
  ...
  const embed = new EmbedBuilder()
    ...
    .addFields(fields)
    .setFooter({
      text: `Use o menu abaixo para ver detalhes por categoria • ${targetUser.username}`,
    })
    .setTimestamp();
...
async function showCategoryItems(interaction, targetUser, category, inventory) {
  ...
  if (userItems.length === 0) {
    const embed = new EmbedBuilder()
      ...
      .setTitle(
        `${storeItemsService.getCategoryIcon(category)} Nenhum Item Encontrado`
      )
      .setDescription(
        `${
          isOwnInventory ? "Você não possui" : "Este usuário não possui"
        } nenhum item na categoria ${storeItemsService.getCategoryDisplayName(
          category
        )}.`
      );
  ...
  // Descrição dos itens
  const itemsDescription = userItems
    .map((item) => {
 ...
 ${item.description}`;
  ...
  if (isOwnInventory) {
    ...
    if (category === "consumiveis") {
      const usableItems = userItems.filter(
        (item) => item.usavel && item.quantity > 0
      );
      ...
      for (let i = 0; i < Math.min(usableItems.length, 5); i++) {
        ...
        const itemRow = new ActionRowBuilder().addComponents(
          new ButtonBuilder()
            ...
            .setEmoji(item.icon)
            .setDisabled(
              item.lastUsed && Date.now() - item.lastUsed < item.cooldown
            )
        );
  ...
  collector.on("collect", async (i) => {
    ...
    if (customId.startsWith("use_")) {
      ...
      await i.followUp({
        content:
          "Esta funcionalidade será implementada com o comando `/usar`. Por favor, use esse comando para utilizar seus itens.",
        ephemeral: true,
      });
    }
  });
