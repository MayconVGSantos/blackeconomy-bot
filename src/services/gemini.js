...
class GeminiClient {
    ...
    while (retries <= maxRetries) {
      try {
        ...
        const timeoutPromise = new Promise((_, reject) => 
          setTimeout(() => reject(new Error('Timeout na requisição para API Gemini')), 10000)
        );
        ...
        lastError = error;
        console.error(`Tentativa ${retries + 1}/${maxRetries + 1} falhou:`, error.message);
        retries++;
  ...
  async gerarDescricaoItem(itemName, itemType, price) {
    ...
    Crie uma descrição curta e atrativa para um item da loja virtual chamado "${itemName}" que custa R$${price.toFixed(2)}.
