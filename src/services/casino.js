...
class CasinoService {
  ...
  generateSlotsResult() {
    ...
    // Aumento da diferença entre os pesos para tornar mais difícil obter símbolos de alto valor
    const weights = [35, 28, 20, 10, 5, 1.5, 0.5]; // Probabilidades ponderadas com valores raros mais difíceis
    ...
    // Gerar 3 símbolos independentes (com dificuldade aumentada para obter combinações)
    ...
    // Três símbolos iguais (muito mais difícil agora)
    ...
    // Dois símbolos iguais (mais difícil também)
    else if (
      reels[0] === reels[1] ||
      reels[1] === reels[2] ||
      reels[0] === reels[2]
    ...
    // Presença de um 7 - taxa reduzida de prêmio
    else if (reels.includes("7️⃣")) {
      // Reduzindo a chance de ganhar com apenas um 7
      if (Math.random() < 0.4) {
        // 40% de chance de ganhar com um 7
